...
gulp.task('copy',  function() {
    return gulp.src(SRC_DIR + 'js/*.js', {base: 'src'}) //保存目录结构
        .pipe(gulp.dest(DEV_DIR))
});

// 编译js 转es5 并压缩
gulp.task('js', function() {
    gulp.src([SRC_DIR + 'js/index.js'], {base: 'src'})
        .pipe(babel({
            presets: ['es2015']
        }))
        .pipe(gulp.dest(DEV_DIR))
...
gulp.task('watch', function() {
    connect.server({
        ...
        middleware: function(connect, opt) {
            return [
                proxy('/platform',  {
                    target: 'http://sandbox-h5sdk.gamdream.com',
                    changeOrigin: true,
                    pathRewrite: {
                        '^/platform': '/platform'
                    }
...
gulp.task('minhtml', function() {
    gulp.src(SRC_DIR + '**/*.html')
        .pipe(cdn({
            domain: /\.\//,
            cdn: "//dl.gamdream.com/activity/galan/pingtai/"
        }))
        // .pipe(clearnHtml())
        .pipe(gulp.dest(DIST_DIR));
});
...
// 监听事件
gulp.task('dev', gulpSequence('clean', ['watch','image', 'css', 'html', 'js', 'copy']));
gulp.task('build', gulpSequence('minclean', ['minimage', 'mincopy', 'mincss', 'minhtml', 'minjs']));